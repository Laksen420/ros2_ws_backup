...
class MultiIMUNode(Node):
    def __init__(self):
        ...
        time.sleep(1.0)  # Increased wait time for I2C to initialize
        
        # Scan I2C bus
        i2c_devices = []
        try:
            if self.i2c.try_lock():
                i2c_devices = [hex(x) for x in self.i2c.scan()]
                self.i2c.unlock()
                self.get_logger().info(f'I2C scan results: {i2c_devices}')
            else:
                self.get_logger().error('Could not lock I2C bus for scanning!')
        except Exception as e:
            self.get_logger().error(f'Error scanning I2C bus: {e}')

        # Initialize both IMUs with retry logic
        self.bno1 = None  # Will be set in init_imu1
        self.bno2 = None  # Will be set in init_imu2
        
        # Try up to 3 times to initialize each IMU
        imu1_ok = False
        imu2_ok = False
        
        for attempt in range(3):
            self.get_logger().info(f'IMU initialization attempt {attempt+1}/3')
            
            if not imu1_ok:
                imu1_ok = self.init_imu1()
                if imu1_ok:
                    self.get_logger().info(f'IMU1 initialized on attempt {attempt+1}')
            
            if not imu2_ok:
                imu2_ok = self.init_imu2()
                if imu2_ok:
                    self.get_logger().info(f'IMU2 initialized on attempt {attempt+1}')
            
            if imu1_ok and imu2_ok:
                break
                
            time.sleep(1.0)  # Wait before retrying

        if not (imu1_ok or imu2_ok):
            self.get_logger().error("Failed to initialize both IMUs!")
        elif not imu1_ok:
            self.get_logger().warn("Failed to initialize IMU1, but IMU2 is working")
        elif not imu2_ok:
            self.get_logger().warn("Failed to initialize IMU2, but IMU1 is working")
        else:
    ...
    def init_imu1(self):
        ...
        try:
            ...
            if self.i2c.try_lock():
                i2c_devices = [hex(x) for x in self.i2c.scan()]
                self.i2c.unlock()
                self.get_logger().info(f'I2C scan before IMU1 init: {i2c_devices}')
            else:
                self.get_logger().warn("Could not lock I2C bus for scanning!")
            
            self.get_logger().info('Initializing first BNO085 IMU at address 0x4A...')
            
            # Additional debug - try a simple read before initialization
            try:
                if self.i2c.try_lock():
                    buffer = bytearray(1)
                    self.i2c.readfrom_into(0x4A, buffer)
                    self.get_logger().info(f"Pre-init test read from 0x4A: {buffer[0]}")
                    self.i2c.unlock()
                else:
                    self.get_logger().warn("Could not lock I2C bus for test read!")
            except Exception as e:
                self.get_logger().error(f"Test read from IMU1 failed: {e}")
            ...
            try:
                ...
                self.get_logger().info(f"IMU1 test read successful: accel={accel}")
            except Exception as e:
                self.get_logger().error(f"IMU1 test read failed: {e}")
    ...
    def init_imu2(self):
        ...
        try:
            ...
            if self.i2c.try_lock():
                i2c_devices = [hex(x) for x in self.i2c.scan()]
                self.i2c.unlock()
                self.get_logger().info(f'I2C scan before IMU2 init: {i2c_devices}')
            else:
                self.get_logger().warn("Could not lock I2C bus for scanning!")
            ...
            try:
                if self.i2c.try_lock():
                    buffer = bytearray(1)
                    self.i2c.readfrom_into(0x4B, buffer)
                    self.get_logger().info(f"Pre-init test read from 0x4B: {buffer[0]}")
                    self.i2c.unlock()
                else:
                    self.get_logger().warn("Could not lock I2C bus for test read!")
            except Exception as e:
                self.get_logger().error(f"Test read from IMU2 failed: {e}")
            ...
            try:
                ...
                self.get_logger().info(f"IMU2 test read successful: accel={accel}")
            except Exception as e:
                self.get_logger().error(f"IMU2 test read failed: {e}")
                raise  # Re-raise to trigger the exception handler
    ...
    def publish_calibration_status(self):
        ...
        try:
            if self.bno1 is not None:
                ...
                calib1_msg = String()
                calib1_msg.data = "IMU1 not initialized or not connected"
                self.calib1_pub.publish(calib1_msg)
        ...
        try:
            if self.bno2 is not None:
                ...
                calib2_msg = String()
                calib2_msg.data = "IMU2 not initialized or not connected"
                self.calib2_pub.publish(calib2_msg)
    ...
    def publish_imu_data(self):
        ...
        if imu1_data and imu2_data:
            ...
            # Just use IMU1 data for combined
            self.combined_pub.publish(imu1_data)
            self.get_logger().debug("Using only IMU1 data for combined output", throttle_duration_sec=5.0)
        elif imu2_data:
            # Just use IMU2 data for combined
            self.combined_pub.publish(imu2_data)
            self.get_logger().debug("Using only IMU2 data for combined output", throttle_duration_sec=5.0)
