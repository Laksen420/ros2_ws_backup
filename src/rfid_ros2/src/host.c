...
{
    if(close((int) port_handle) < 0) return (-1);
    return (0);
}
...
int16_t _tx(void* port_handle, uint8_t* data, uint32_t len)
{
    ...
    while(1)
    {
        if((bytes_sent = write((int) port_handle, &data[idx],
...
{          
    ...
    while(1)
    {
        if((bytes_received = read((int) port_handle, &data[idx], 
                            bytes_to_receive)) < 0)
        {
            return (-1);
        } 
...
{
    //Flush port Input
    if(tcflush((int) port_handle, TCIFLUSH) < 0) return (-1);
    return (0);
}
